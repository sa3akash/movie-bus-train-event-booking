  ...
  return (
    <div className="flex-1 bg-muted/20 p-8 rounded-3xl border-4 border-muted/50 flex flex-col items-center shadow-inner relative min-h-[600px] overflow-hidden">
      ...
      <div
        ...
        style={{
          gridTemplateColumns: `auto repeat(${customCols}, minmax(0, 1fr))`,
        }}
      ...
      >
        ...
        {Array.from({ length: customCols }).map((_, x) => (
          <div
            key={`header-col-${x}`}
            className="flex items-center justify-center text-xs font-bold text-muted-foreground/60 select-none"
          >
            {x + 1}
          </div>
        ))}
