...
import { ArrowLeft, Save, BusFront } from 'lucide-react'
import Link from 'next/link'

const mockBrands = ['Green Line', 'Shyamoli', 'Hanif Enterprise', 'Ena', 'S.Alam']
const mockTypes  = ['AC Sleeper', 'AC Seater', 'Non-AC Seater', 'Double Decker']
    ...
    <div className="flex-1 space-y-6 p-8 pt-6 max-w-3xl">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-muted-foreground mt-0.5">Register a new bus to the fleet</p>
        </div>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Brand and bus type assignment</CardDescription>
        </CardHeader>
        <CardContent className="grid grid-cols-2 gap-4">
          <div className="space-y-2">
            <Label>Brand *</Label>
            <Select value={form.brandId} onValueChange={(v) => setForm((f) => ({ ...f, brandId: v }))}>
              ...
              <SelectContent>
                {mockBrands.map((b) => <SelectItem key={b} value={b}>{b}</SelectItem>)}
              </SelectContent>
            </Select>
          </div>
          <div className="space-y-2">
            ...
            <Select value={form.typeId} onValueChange={(v) => setForm((f) => ({ ...f, typeId: v }))}>
              ...
              <SelectContent>
                {mockTypes.map((t) => <SelectItem key={t} value={t}>{t}</SelectItem>)}
              </SelectContent>
            </Select>
          </div>
          <div className="space-y-2">
      ...
      <div className="flex justify-end gap-3">
        ...
        <Button className="gap-2" onClick={() => router.push('/admin/buses')}>
