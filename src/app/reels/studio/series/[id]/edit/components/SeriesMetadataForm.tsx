...
export function SeriesMetadataForm({ seriesId, initialData, onSuccess }: SeriesMetadataFormProps) {
  const [loading, setLoading] = useState(false);
  const [title, setTitle] = useState(initialData?.title || "");
  const [description, setDescription] = useState(initialData?.description || "");
  const [genre, setGenre] = useState(initialData?.genre || "");
  const [language, setLanguage] = useState(initialData?.language || "en");
  const [ageRating, setAgeRating] = useState(initialData?.ageRating || "PG-13");
  const [director, setDirector] = useState(initialData?.director || "");
  const [releaseYear, setReleaseYear] = useState(initialData?.releaseYear || new Date().getFullYear());
  const [isPremium, setIsPremium] = useState(initialData?.isPremium || false);
  const [coinPrice, setCoinPrice] = useState(initialData?.defaultPricePerEpisode || 10);
  const [castMember, setCastMember] = useState("");
  const [cast, setCast] = useState<string[]>(initialData?.cast || []);
  const [coverImage, setCoverImage] = useState<{ url: string; id?: string } | null>(
    initialData?.coverImage?.url
      ? { url: initialData.coverImage.url, id: initialData.coverImageId }
      : initialData?.coverImageId
      ? { url: `/api/images/${initialData.coverImageId}`, id: initialData.coverImageId }
      : null
  );
          ...
          <div className="w-full max-w-[200px] aspect-[2/3] bg-black border-2 border-dashed border-white/20 rounded-xl relative overflow-hidden">
            {coverImage?.url ? (
              ...
              <div className="absolute inset-0 [&>div]:h-full [&>div]:w-full [&>div>div]:border-none [&>div>div]:bg-transparent">
                <ImageUpload value={coverImage as any} onChange={(img) => setCoverImage(img as any)} />
              </div>
            )}
          </div>
        </div>
