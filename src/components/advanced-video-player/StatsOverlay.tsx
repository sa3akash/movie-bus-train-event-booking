    ...
    const updateStats = () => {
      ...
      if (video && video.buffered.length > 0) {
        ...
        for (let i = 0; i < video.buffered.length; i++) {
          if (
            current >= video.buffered.start(i) &&
  ...
  return (
    <>
      ...
      >
        {/* Mobile Handle */}
        <div
          className="w-full flex justify-center pb-4 md:hidden shrink-0"
          onClick={onClose}
        >
        ...
        <div className="flex flex-col gap-2 overflow-y-auto overscroll-contain pr-2 pb-4 md:pb-0">
          ...
          <div className="flex justify-between">
            ...
            <span>
              {stats.playerWidth}x{stats.playerHeight} /{" "}
              {stats.droppedFrames || 0} dropped
            </span>
          </div>
          <div className="flex justify-between">
            ...
            <span>
              ...
              {stats.width || stats.videoWidth}x
              {stats.height || stats.videoHeight}
            </span>
          </div>
          <div className="flex justify-between">
          ...
          <div className="flex justify-between">
            ...
            <span
              className="truncate max-w-[180px] text-right"
              title={`${stats.videoCodec} / ${stats.audioCodec}`}
            >
          ...
          <div className="flex justify-between">
            ...
            <span>{Math.round((stats.streamBandwidth || 0) / 1000)} Kbps</span>
          </div>
          <div className="flex justify-between">
            <span className="text-white/60">Buffer Health</span>
            <span>{(stats.bufferHealth || 0).toFixed(2)} s</span>
          </div>
          {stats.isLive && (
            <div className="flex justify-between">
              <span className="text-white/60">Live Latency</span>
              <span>{(stats.liveLatency || 0).toFixed(2)} s</span>
            </div>
          )}
          <div className="flex justify-between">
            <span className="text-white/60">Latency / Play / Buffer</span>
            <span>
              {Math.round((stats.loadLatency || 0) * 1000)}ms /{" "}
              {Math.round(stats.playTime || 0)}s /{" "}
              {Math.round(stats.bufferingTime || 0)}s
            </span>
          </div>
        </div>
      </div>
